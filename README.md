...
澜页是一个开源、本地优先的 AI 阅读笔记 MVP：用户上传文档后进行固定翻页式阅读，系统用摄像头视线追踪估计页内停留区域，再把“已阅读页原文上下文 + 阅读行为线索 + 用户笔记需求”交给 AI 生成阅读笔记。

关键词：AI 笔记、阅读笔记、学习笔记、视线追踪、眼动追踪、PDF 阅读器、文档笔记、复习助手。

> 当前项目定位为非商业开源原型。数据默认保存在本地；音频和摄像头画面不进入上传流程。公开贡献使用示例配置和脱敏材料。

> 当前摄像头路线是一项需要验证的产品假设：它的价值在于低打扰地记录阅读行为并服务未来用户画像，而不是宣称普通摄像头能直接理解用户意图。仓库同时保留无摄像头/手动模式作为对照和降级方向。
...
这个 MVP 面向学习阅读场景：在阅读时用摄像头追踪视线，形成可回看的阅读行为线索，再生成更贴近真实阅读过程的笔记或复习建议。

它的设计目标是让 AI 提供建议、指出盲区、帮助复盘，同时保留用户主动思考的位置。视线轨迹更适合作为回看线索；用户的主动选中、标注、提问和反馈是更可靠的明确动作。
...
- 固定翻页支持按钮、方向键、PageUp/PageDown、空格和鼠标滚轮；滚轮翻页带节流，避免连续滚动扰乱视线布局。
...
- 只把本次会话实际阅读过的页提交给 AI。
- 结果会先展示程序生成的证据面板，再展示 AI 辅助笔记；视线停留只影响回看建议和自测问题，不直接证明理解或忽略。
- 支持通过环境变量配置 AI provider；缺少 API Key 时回退到本地规则版。

## 当前未实现或仍在研究

- 用户画像：当前只保存阅读目的和笔记偏好，还没有长期画像学习。
- 走神提醒：未实现。未来必须低频、可关闭，并区分走神与有价值思考。
- 眼部舒适度：未实现疲劳检测，也不做医学判断；只适合先研究采样时长、休息提醒和可关闭策略。
- 证据预览/热区图：当前生成后有证据面板，生成前确认和热区可视化仍未实现。
- 无摄像头模式：当前可在没有 gaze 时依靠原文、停留和主动选中生成结果，但还不是完整的手动模式。
- OCR、公式、表格、图片区域识别：当前主要适合文字型 PDF 和常见文本文件。
- 托盘/悬浮窗/浏览器扩展、本地模型、会话管理界面：都是后续方向。
...
4. 回到浏览器进行固定翻页式阅读，可用按钮、方向键、PageUp/PageDown、空格或鼠标滚轮翻页。
5. 阅读时可用选中文字作为主动标注。
...
重点方向包括：更精细的 gaze 延迟校准、扫描版 PDF OCR、托盘/悬浮窗形态、用户画像与笔记风格、走神提醒、主动标注工作流、眼部舒适度研究（未实现、非医学判断）、安全边界、社区插件化。
